...
#define SUPERMICRO_X7DBT_SENSORS_CNT	24


struct nm_supermicro_x7dbt_sensors_description_t {
	uint16_t			nmsensd_fw_version;
	uint16_t			sensors_cnt;
	struct nm_sensor_desc_t		sensors[SUPERMICRO_X7DBT_SENSORS_CNT];
	uint32_t			crc32;
} __attribute__((packed));


struct nm_supermicro_x7dbt_sensors_description_t platform_description =
{
	.nmsensd_fw_version	= NMSENSD_FW_VERSION,
	.sensors_cnt		= SUPERMICRO_X7DBT_SENSORS_CNT,
	.sensors = {
